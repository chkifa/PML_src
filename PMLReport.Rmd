...
The model was then fit using all the available predictors (except for the outcome), the *OOB* (Out-Of-Bag) as error estimate, **accuracy** as metric, the default number of *500* trees and enabling the proximity matrix.
...
As shown by the model summary, the highest accuracy of **99%** was reached with 23 randomly sampled variables (at each split), this is also depicted by the figure below.
...
The plot below shows instead the error with respect to the number of used trees, it is clear from the graphs how slowly the error converges to zero as the number of trees increases.
